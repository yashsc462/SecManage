...
class SalaryDetails(models.Model):
    ...
    #   # New fields for advance payment details
    # advance_payment = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True)
    # advance_request_status = models.CharField(max_length=20, null=True, blank=True)
    # advance_payment_approval_date = models.DateField(null=True, blank=True)
...
class AdvancePayment(models.Model):
    employee = models.ForeignKey('EmployeeJoining', on_delete=models.CASCADE)  # Link to Employee
    salary_details = models.ForeignKey('SalaryDetails', on_delete=models.CASCADE)  # Link to SalaryDetails
    ...
    approval_pdf = models.FileField(upload_to='advance_pdfs/', null=True, blank=True)
    
    def __str__(self):
        return f"Advance Payment of {self.advance_amount} for {self.employee}"
